...
- `grpc` contains settings for grpc connections
  - `use_resource_exhausted_code` By default, Emissary will return an `UNAVAILABLE` gRPC code when a request is rate limited.
  When set to `true`, this field will cause Emissary will return a `RESOURCE_EXHAUSTED` gRPC code instead.
