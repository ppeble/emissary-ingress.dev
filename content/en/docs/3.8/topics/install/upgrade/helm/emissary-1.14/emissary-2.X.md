...
<Alert severity="info">
  This guide covers migrating from Emissary 1.14.X to Emissary 2.x. If
...
We're pleased to introduce Emissary 2.x! The 2.X family introduces a number of
...
2.x side-by-side in the same cluster. This gives Emissary 2.x
...
1. **Emissary 1.14 will not see any `getambassador.io/v3alpha1` resources.**
   ...
   Emissary 2.x, while not interfering with the operation of your
   Emissary 1.14 installation.

2. **If needed, you can use labels to further isolate configurations.**

   If you need to prevent your Emissary 2.x installation from
   ...
   your Emissary 2.x installation, then set its
   ...
   that should be visible to Emissary 2.x, then set
...
3. **Be careful about label selectors on Kubernetes Services!**
   ...
   Pods from Emissary 2.x, traffic will be erroneously split between
   Emissary 1.14 and Emissary 2.x. The labels used by Emissary
   2.x include:
...
4. **Be careful to only have one Emissary Agent running at a time.**
   ...
   Emissary 2.x. Once testing is done, you can switch Agents safely.

You can also migrate by [installing Emissary 2.x in a separate cluster](../../../../migrate-to-2-alternate).
This permits absolute certainty that your Emissary 1.14 configuration will not be
affected by changes meant for Emissary 2.x, and it eliminates concerns about
...
2. **Install new CRDs.**
   ...
   Before installing Emissary 2.x itself, you must configure your
   ...
   must install support for `getambassador.io/v3alpha1`** to run Emissary 2.x,
   even if you intend to continue using only `getambassador.io/v2` resources for some
   time.

   ```
   kubectl apply -f https://app.getambassador.io/yaml/emissary/2.x/emissary-crds.yaml
   kubectl wait --timeout=90s --for=condition=available deployment emissary-apiext -n emissary-system
   ```

   <Alert severity="info">
     Emissary 2.x includes a Deployment in the `emissary-system` namespace
...
3. **Install Emissary 2.x.**

   After installing the new CRDs, you need to install Emissary 2.x itself
   **in the same namespace as your existing Emissary 1.14 installation**. It's important
   ...
   <Alert severity="info">
     Emissary 2.x includes a Deployment in the $productNamespace$ namespace
     called <code>emissary-apiext</code>. This is the APIserver extension
     that supports converting Emissary CRDs between <code>getambassador.io/v2</code>
     and <code>getambassador.io/v3alpha1</code>. This Deployment needs to be running at
     all times.
...
4. **Install `Listener`s and `Host`s as needed.**
   ...
   An important difference between Emissary 1.14 and Emissary 2.x is the
   new **mandatory** `Listener` CRD. Also, when running both installations side by side,
   you will need to make sure that a `Host` is present for the new Emissary 2.x
   Service. For example:
...
5. **Test!**
   ...
   Your Emissary 2.x installation can support the `getambassador.io/v2`
   ...
   <Alert severity="info">
    ...
    <br/>
    If you find that your Emissary 2.x installation and your Emissary 1.14
   ...
   **If you find that you need to roll back**, just reinstall your 1.14 CRDs and delete your
   installation of Emissary 2.x.

6. **When ready, switch over to Emissary 2.x.**

   You can run Emissary 1.14 and Emissary 2.x side-by-side as long as you care
   ...
   since some useful features in Emissary 2.x are only available using
   `getambassador.io/v3alpha1` resources.

   When you're ready to have Emissary 2.x handle traffic on its own, switch
   your original Emissary 1.14 Service to point to Emissary 2.x. Use
...
7. **Finally, install the Emissary 2.x Ambassador Agent.**
...
Congratulations! At this point, Emissary 2.x is fully running and it's safe to remove the `ambassador` and `ambassador-agent` Deployments:
