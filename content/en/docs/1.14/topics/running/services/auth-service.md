...
| Attribute | Default value | Description |
| --- | --- | --- |
|`protocol_version` | `v2` | gRPC service name used to communicate with the AuthService. Allowed values are `v2` which will use the `envoy.service.auth.v2.Authorization` service name, and `v3` which will use the `envoy.service.auth.v3.Authorization` service name. `v3` requires Emissary to run in Envoy v3 mode by setting the [AMBASSADOR_ENVOY_API_VERSION=V3 environment variable](../../running/#ambassador_envoy_api_version).|
...
An AuthService can be disabled for a Mapping by setting `bypass_auth` to `true`. This will tell Emissary to allow all requests for that Mapping through without interacting with the external auth service.
