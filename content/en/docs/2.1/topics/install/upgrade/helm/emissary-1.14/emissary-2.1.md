...
3. **Install $productName$ $version$.**
   ...
   **in the same namespace as your existing $productName$ 1.14 installation**. It's important
   ...
   You should install $productName$ $version$ using Helm if you installed $productName$ 1.14
   with Helm, or by manually applying YAML if not. To check if you installed $productName$ 1.14
   ...
   - If Helm returns resources, continue with [Helm](../../../../helm/#install-with-helm). **Note that if your $productName$ 1.14
   ...
   - If Helm does not return resources, continue with YAML. **Note that if your $productName$
     1.14 installation uses a nonstandard namespace, or if you need to set `AMBASSADOR_LABEL_SELECTOR`,
