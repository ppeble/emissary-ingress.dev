...
#### The request From Emissary to the ExtAuth service
...
The request Emissary will make of the auth service is:
...
#### The response returned from the ExtAuth service to Emissary

 - If the HTTP response returned from the ExtAuth service to Emissary has an HTTP status code of 200, then the request is allowed through to the upstream backend service.  **Only** 200 indicates this; other 2XX status codes will prevent the request from being allowed through.
